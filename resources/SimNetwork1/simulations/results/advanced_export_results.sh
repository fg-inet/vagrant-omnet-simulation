...
do
    ...
    do
        ...
        scavetool export -f "(module(*.ue[$cli_num].tcpApp[0]) AND name(*endToEndDelay*)) OR (module(*.ue[$cli_num].tcpApp[0]) AND name(*DASH*)) OR (module(*.ue[$cli_num].tcpApp[0]*) AND name(*rcvdPk*)) OR (module(*.ue[$cli_num].tcpApp[0]*) AND name(*sentPk*)) OR (module(*.ue[$cli_num].tcp) AND name(\"*measured RTT*\")) OR (module(*.ue[$cli_num].tcp) AND name(\"*smoothed RTT*\"))" -F CSV -o ${resultsFolder}/${subfolderName}/vectors/${experimentDescriptor}_${num_ue}_cli${cli_num}_vec.csv ${sourceFolder}/*-${run_num}.vec
        scavetool export -f "(module(*.ue[$cli_num].tcpApp[0]) AND name(*Pk:sum*)) OR (module(*.ue[$cli_num].*) AND name(*Cqi*)) OR (module(*.ue[$cli_num].*) AND name(*position*))" -F CSV -o ${resultsFolder}/${subfolderName}/scalars/${experimentDescriptor}_${num_ue}_cli${cli_num}_sca.csv ${sourceFolder}/*-${run_num}.sca
    done
done
